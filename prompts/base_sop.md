...
- **Decision Gate:**
  ...
  - IF all required tools and data are available, THEN proceed to Step 3.
...
- **Decision Gate:**
  - IF a tool call fails, THEN retry once. If it fails again, set status to `failed` with the error summary.
