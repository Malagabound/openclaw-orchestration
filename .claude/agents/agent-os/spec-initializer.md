...
Return or output the following (replace `[DATED_SPEC_NAME]` with the actual dated spec name like `2025-12-05-my-feature`):

```
Spec folder initialized!

Spec name: `[DATED_SPEC_NAME]`
Location: `agent-os/specs/[DATED_SPEC_NAME]/`
...
- All specs are stored in `agent-os/specs/` (NEVER `.claude/specs/` or any other location)
- Always use dated folder names (YYYY-MM-DD-spec-name)
- Pass the **spec name** (e.g., `2025-12-05-my-feature`) back to the orchestrator - NOT the full path
